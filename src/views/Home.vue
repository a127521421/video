<template>
  <div class="home">
    <div class="flex">
      <div class="card" v-for="(video, idx) in videos.slice(pageStart, pageStart + countOfPage)" :key="idx">
        ...
        <button v-if="video.Favorite" class="deleteFavoritebutton" @click="dele(video)">
...
export default {
  ...
  computed: {
    ...
    totalPage: function () {
      return Math.ceil(this.videos.length / this.countOfPage)
    },
    Favorites () {
  ...
  methods: {
    ...
    dele (video) {
      ...
      this.$store.commit('deleVideo', video)
    },
    jump (id) {
